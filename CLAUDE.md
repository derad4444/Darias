...
### 3.5. 機能を変えたら、影響する場所をすべて直す（仕様書は毎回・画像と公開ページはリリース時）

**機能の追加・削除・名前や見た目の変更をしたら、仕様書だけでなく、その機能を説明・宣伝・撮影している場所も直す。**
アプリの外の説明は grep に引っかからない別リポジトリや画像にあることが多く、放っておくと古い説明が公開され続ける
（実例: 削除した冒険機能を LP が9月下旬まで宣伝していた／サポートページが「BIG5で100問に答える」のままだった／
リールの画面素材に削除済みの冒険タブが写っていた／LP の画像にテスト広告が写り、法務ページは2024年版の写しのままだった）。

直す場所は、**いつ直すか**で次の2つに分ける。

#### A. その作業の中で毎回直すもの（コードと一緒に変わるもの）

| 置き場所 | 何があるか | 直し方 |
|---|---|---|
| 仕様書（各リポジトリ） | DARIAS 本体の `shared/docs/`・LP の `~/dev/DARIASLP/docs/LP仕様書.md`・リール制作ツールの `~/dev/リール制作ツール/アプリ/自動投稿/docs/` | 変えたリポジトリの仕様書を同じコミットで更新する。直したのに仕様書が古いと、次に直す人が古い前提で作業する |
| アプリ内の説明 | 使い方ガイド（`help_guide_screen.dart`）・オンボーディング（`onboarding_screen.dart`）・ヒント（`hint_service.dart`） | コードを直してアプリを再起動して確認する |
| アプリ内の法務の写し | `terms_of_service_screen.dart`・`privacy_policy_screen.dart` | 正本（`shared/docs/public/`）と必ず対で直す |

さらに、**見た目や説明が変わる変更をしたら、`shared/docs/release/リリース手順書.md` の「次のリリースで確認すること」に1行足す**
（何を変えたか・どの画像や説明に影響しそうか）。リリースのときにこの一覧を見て B を行う。

#### B. リリースのたびに1回まとめて直すもの（画像と公開ページ）

ストア画像（iPhone・iPad）・LP の画面画像と文章・リールの画面素材と台本の前提・Instagram のハイライト・サポートページ。
**手順とタイミングは `shared/docs/release/リリース手順書.md` の「リリース時の画像・公開ページの確認」に従う。**
機能を変えるたびに作り直すと手間が大きいため、ここでは直さない。反映（デプロイ・push・投稿）は**審査に提出した時点**で行う（公開のタイミングは読めないため）。

どちらの場合も:
- 変える機能の名前・画面名・文言で検索する（別リポジトリの LP・リール制作ツールも含める）。画像や動画は検索に出ないので目で確認する
- ユーザーの手作業が要るもの（ストアの説明文・Instagram・push）は、最後にまとめて一覧で報告する
- 利用規約・プライバシーポリシーの写しを別の場所に置かない。正本は `shared/docs/public/` で、他からはその公開ページをリンクする
