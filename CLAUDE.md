...
| Hosting 3サイト | Web版アプリ・配布ページ（`dl/`）・LP（`~/dev/DARIASLP/`・**別リポジトリ**。仕様書は `DARIASLP/docs/LP仕様書.md`） | それぞれデプロイ。LPは grep から漏れるので必ず見る。LPの画面画像はストア画像のコピーなので、ストア画像を作り直したらLPにもコピーする。確認は `curl` で公開中の中身を見る |
| ストア | ストア画像（生成スクリプト `~/dev/DARIAS/画像/ストア画像/生成スクリプト/` の `make_shots.py`・`make_ipad_shots.py`。素材は `画像/画面スクショ/<日付>/`）・説明文 | 素材をシミュレータで撮り直して生成する（**iPhone と iPad の両方**）。App Store Connect / Google Play への画像のアップロードと説明文はユーザーが手で直す（リンクを添えて案内する） |
| リール制作ツール | `~/dev/リール制作ツール/アプリ/自動投稿/` の画面素材（`assets/screens/` と `screens.json`）・台本の前提（`src/content/claude_content.py` の元素の性格・判定軸・機能の説明）。仕様書は同ツールの `docs/` | 画面はシミュレータで撮り直して差し替え、Cloud Run にデプロイ |
| Instagram | ハイライト（はじめまして・性格タイプ・AIと話してみた・使い方）・固定表示のリール | 画像や動画を作り直し、投稿と差し替えはアプリで手作業（ユーザーに手順を渡す） |

**それぞれのリポジトリの仕様書も、同じ作業の中で更新する。** DARIAS 本体の `shared/docs/`、LP の `DARIASLP/docs/LP仕様書.md`、
リール制作ツールの `docs/`（型とデザイン・コスト）。直したのに仕様書が古いままだと、次に直す人が古い前提で作業する。
...
3. **見た目が変わった機能**（タブの追加・削除、画面構成の変更など）は、リールの画面素材・ハイライト・ストア画像（iPhone・iPad）・LPの画面画像に古い画面が写っていないかを必ず見る
4. 画像を作り直すときは、**テスト広告（「Test mode」のバナー）・削除済みの機能・個人情報（メールアドレス・日記の本文など）**が写っていないかも見る
5. 利用規約・プライバシーポリシーの写しを別の場所に置かない（LPに2024年版の写しが残っていた）。正本は `shared/docs/public/` で、他からはその公開ページをリンクする
