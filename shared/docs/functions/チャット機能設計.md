...
メッセージから予定情報を抽出（**複数件対応**）。抽出結果はFlutter側で確認ダイアログを経てFirestoreに保存。

**入力:**
```javascript
{
  userId: string,
  userMessage: string,  // 複数の予定を含むテキストも可
}
```
  ...
  schedules: [          // 常に配列（0件の場合は空配列）
    {
      ...
      remindValue: number,    // 0
      remindUnit: string,     // "none"
...
**件数ごとの動作:**
- `schedules.length === 0` → 通常チャット（`generateCharacterReply`）にフォールバック
- `schedules.length >= 1` → 確認ダイアログ（`_ScheduleListConfirmDialog`）を表示

### Flutter側: `SendMessageResult` クラス

```dart
class SendMessageResult {
  final String? replyText;
  final List<ScheduleModel> detectedSchedules; // 複数件対応（空リストは未検出）
  bool get scheduleDetected => detectedSchedules.isNotEmpty;
  // ...
}
```

### 確認ダイアログ: `_ScheduleListConfirmDialog`
