...
### classifyAndExtract の費用（全メッセージで実行）

```
gpt-4o-mini 固定:
- 入力: プロンプト(~300t) + メッセージ(~50t) = ~350 tokens
- 出力: ~100 tokens（分類結果のJSON）
- コスト: 350 × $0.15/1M + 100 × $0.60/1M = $0.052 + $0.06 / 1000回
        = $0.000112/回 ≈ 0.017円/回
```
...
### 旧 extractSchedule との比較

| 方式 | 全メッセージの処理 | 予定メッセージの追加処理 |
|-----|----------------|---------------------|
| 旧（キーワード+extractSchedule） | なし | ~0.02円/回 |
| 新（classifyAndExtract） | ~0.017円/回 | なし（抽出込み） |

全メッセージでAI分類コストが発生するが、キーワードベース判定の誤検知解消と複数件抽出の恩恵が得られる。
