...
**操作:**
- **[編集]**: ダイアログを閉じてカレンダー詳細画面（`/calendar/detail`）へ遷移。ScheduleModelを `extra` に渡す
- **[除外]**: `setState()` でリストから除去（即時反映）。全件除外でリスト空になると「全て除外されました」メッセージ＋[閉じる]ボタンに切り替わる
- **[X件追加する]**: `_saveScheduleModels(List<ScheduleModel>)` を呼び出し全件をFirestoreに保存。成功数を `SnackBar` で通知
