...
- **ホーム画面ウィジェット**: WidgetKit (SwiftUI) で実装
  ...
  - ウィジェット種別: カレンダー（今日/明日/明後日の予定）、メモ、Todo、Big5進捗
  - カラー定義: `WidgetColors.swift` でアプリのピンク/ラベンダー配色を統一
...
- **ホーム画面ウィジェット**: `AppWidgetProvider` (Kotlin) で実装
  ...
  - レイアウト: `android/app/src/main/res/layout/` 配下のXMLで定義
  - データ連携: `home_widget` パッケージ経由でSharedPreferencesに書き込み
