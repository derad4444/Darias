...
│   │   ├── screens/                   # 画面
│   │   │   ├── main/
│   │   │   │   └── main_shell_screen.dart     # 5タブ（ホーム / 詳細 / フレンド / 自分会議 / 設定）
│   │   │   ├── splash/
│   │   │   ├── onboarding/
│   │   │   ├── auth/
│   │   │   │   ├── login_screen.dart
...
| フレンド | 検索・申請・承認、フレンド詳細（相性診断／聞いてみるの2タブ） |
| 日記 | 日記 CRUD |
| 課金 | サブスクリプション管理 |
| 広告 | バナー広告、リワード広告 |
...
下部タブバーは5タブ（ホーム / 詳細 / フレンド / 自分会議 / 設定）。
実装は `main_shell_screen.dart` の `IndexedStack` と自前のタブバーで、
`selectedTabProvider`（`StateProvider<int>`）が現在のタブを持つ。

**タブごとに独立した画面スタックを持つ。** 各タブの中身はそのタブ専用の `Navigator`
（`_tabNavigatorKeys`）の最初のルートとして表示し、タブの中から開いた画面は
`Navigator.push` でそのタブのスタックに積む。こうすることで、開いた先でも
タブバーが下に残り、他のタブへ移動できる。

| 開き方 | タブバー | 対象 |
|--------|---------|------|
| タブ内の `Navigator.push` | 残る | フレンド追加・申請管理・フレンド詳細・質問履歴・履歴・通知設定・背景色文字色・音量設定・ログイン方法・使い方ガイド・お問い合わせ・利用規約・プライバシーポリシー・性格変動履歴 |
| root の `context.push` / `context.go`（GoRouter） | 消える | スプラッシュ・ログイン・新規登録・パスワード再設定・初期セットアップ・課金画面・オンボーディング |

タブの中身と同じ画面に単独ルートを持たせない方針は維持する（単独ルートで開くと
タブバーを失って他の画面へ移動できなくなるため）。

**戻る操作**: `MainShellScreen` の `PopScope` が Android の戻るを受け、
「今いるタブの中を戻る → 戻れなければホームタブへ → ホームの最初の画面ならアプリを閉じる」
の順に処理する。同じタブをもう一度タップすると、そのタブの最初の画面まで戻る。

### 主要画面の詳細

#### 日記自動生成（scheduledDiaryGeneration / generateDiary.js）
- **生成タイミング**: 毎日 23:50 JST のCloud Functionsスケジュール実行
- **入力データ**: チャット・6人会議・性格診断進捗
- **factsはコードで組み立てる**: 「今日やったこと」（`facts`）はAIに書かせず、`generateDiary.js` がFirestoreの実データから直接生成する。AIが出力するのは `ai_comment` のみ（`{"ai_comment":"..."}`）
  ...
  - 生成されるfactsは順に `デイリーミッションをクリアした` / `会話を${n}件やりとりした` / `「${concern}」について相談した`
