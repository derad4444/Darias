...
factsは今日の活動を事実ベースで2〜5件。「会話」「相談」「性格診断」「冒険」のいずれかのラベルを使い、「ルーチン」「習慣」などのラベルは使わないこと（例:「冒険『対人関係の悩み』に挑んだ」「性格診断を5問回答した」「相談で答えを得た」）。
ai_commentは以下のルールで250〜350文字で作成:
- 上記の口癖・話し方・性格特性を必ず反映したキャラクターらしいトーンで書く
- 今日の活動に具体的に触れ、夢や強みを絡めて前向きに締める
...
テキスト内の予定を全て抽出すること。日時+行動の組合せのみ予定あり。挨拶/感嘆/質問のみは予定なし。
...
出力（予定が1件でも複数件でも必ず配列形式で返すこと）:
予定なし:{"schedules":[]}
予定あり:{"schedules":[{"title":"行動内容","isAllDay":bool,"startDate":"ISO+09:00","endDate":"ISO+09:00","location":"","tag":"","memo":"","repeatOption":"none","remindValue":0,"remindUnit":"none"},...]}
